...
class AccountFollowupReport(models.AbstractModel):
    ...
    def _get_followup_report_lines(self, options):
        ...
        line_num = 0
        # Sort by date_maturity ascending (earliest first) instead of default newest-first
        for l in partner.unreconciled_aml_ids.sorted(
            key=lambda aml: aml.date_maturity or aml.date,
        ).filtered(lambda aml: not aml.currency_id.is_zero(aml.amount_residual_currency)):
            if (
                l.company_id
                in self.env.company.search(
                ...
                lines.append(
                    {
                        ...
                        "unfoldable": False,
                        "columns": [
                            isinstance(v, dict) and v or {"name": v, "template": "account_followup.line_template"}
                            for v in columns
                        ],
                    }
                )
